...
def fetch_medal_tally(df, year, country):
    ...
    flag = 0
    if year == 'Overall Analysis' and country == 'Overall Analysis':
        temp_df = medal_df
    if year == 'Overall Analysis' and country != 'Overall Analysis':
        flag = 1
...
def year_country_list(df):
    ...
    years.sort()
    years.insert(0,'Overall Analysis')
...
def most_successful(df, sport):
    ...
    x = temp_df[temp_df['Medal'].notna()]
    x = x['Name'].value_counts().reset_index()
    x.columns = ['Name', 'Medal Count']  # Rename properly
    x = x.merge(df, on='Name', how='left')
    x.drop_duplicates(['Name','Sport'], inplace=True)
    return x[['Name', 'Medal Count', 'Sport', 'region']].head(20)
...
def most_successful_countrywise(df,country):
    ...
    x = x.drop_duplicates('Name')[['Name', 'Medal Count', 'Sport', 'region']]
    return x
